...
def _dataset_path(dataset: str) -> Path:
    ...
    if normalized == "sample": return DATA_FILE
    if normalized == "historical": return HISTORICAL_FILE
...
def _bars(dataset: str = "sample"):
    return dataframe_to_bars(_frame(dataset))
...
def data_quality(dataset: str = Query("sample")):
    try:
        ...
        return {"dataset": dataset, "status": "passed" if report.passed else "review", "quality": report.__dict__, "report": report.__dict__}
    except Exception as exc:
        return {"dataset": dataset, "status": "unavailable", "quality": None, "report": None, "reason": str(exc)}


@app.get("/api/backtest")
def backtest(risk_percent: float = Query(1.0, gt=0, le=2), risk_reward: float = Query(2.0, gt=0, le=10), dataset: str = Query("sample")):
    try:
        bars = _bars(dataset)
    except Exception as exc:
    ...
    return {"summary": calculate_performance(trades), "trades": [t.to_dict() for t in trades], "bars": bars, "settings": {"risk_percent": risk_percent, "risk_reward": risk_reward, "execution_timeframe": "3m", "dataset": dataset}}
...
def execution_state(risk_reward: float = Query(2.0, gt=0, le=10), dataset: str = Query("sample")):
    ...
    return {"status": "qualified" if confirmation.qualified else "waiting", "dataset": dataset, "anchor": "09:45 MAIN", "reference_high": high, "reference_low": low, "execution_timeframe": "3m", "direction": confirmation.direction, "sweep_type": confirmation.sweep_type, "confirmation_time": confirmation.confirmation_time.isoformat() if confirmation.confirmation_time else None, "entry": entry, "invalidation": invalidation, "target": target, "risk_per_unit": risk_per_unit, "risk_reward": risk_reward, "displacement": confirmation.displacement.relative_strength if confirmation.displacement else None, "structure": confirmation.structure.classification if confirmation.structure else None, "reason": confirmation.reason}


@app.get("/api/historical-data")
def historical_data(dataset: str = Query("historical")):
    if dataset.lower() == "web":
        try:
            frame = _frame("web")
            return {"status": "ready" if not frame.empty else "empty", "instrument": "^GSPC", "proxy_for": "US500.F", "timezone": "America/New_York", "source": "Yahoo Finance public web", "model_hours": "08:45-15:45", "total": summarize_historical_data(frame).__dict__}
        except Exception as exc:
            raise HTTPException(status_code=503, detail=f"Public web data unavailable: {exc}") from exc
    frame = load_historical_csv(HISTORICAL_FILE if HISTORICAL_FILE.exists() else DATA_FILE)
    ...
    return {"status": "ready" if not frame.empty else "empty", "instrument": "US500.F", "timezone": "America/New_York", "source": "repository_historical_csv", "model_hours": "08:45-15:45", "total": summary.__dict__, "model_period": model_summary.__dict__}
