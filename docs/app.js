...
function renderHistoricalData(data = {}) {
  ...
  setText("data-candles", total.rows ?? "—");
  setText("data-sessions", total.sessions ?? "—");
  setText("data-timeframe", total.timeframe_minutes ? `${total.timeframe_minutes} min` : "—");
  setText("data-model-candles", model.rows ?? "—");
}

async function loadHistoricalData() {
  try {
    const response = await fetch(`${API}/api/historical-data`);
    if (!response.ok) throw new Error(`Historical data failed (HTTP ${response.status})`);
    renderHistoricalData(await response.json());
  } catch (error) {
    setText("data-status", "ERROR");
    setText("data-source", error.message);
  }
}
...
function renderExecutionState(state = {}) {
  setText("ah", num(state.reference_high)); setText("al", num(state.reference_low));
...
function renderChart(bars = [], state = {}) {
  ...
  const points = bars.map((bar) => ({ time: parseTime(bar.timestamp), close: Number(bar.close) })).filter((p) => p.time && Number.isFinite(p.close));
  if (points.length < 2) return;
  const width = 1000, height = 350, padX = 25, padY = 25;
  const prices = points.map((p) => p.close);
  [state.reference_low, state.reference_high, state.entry, state.invalidation, state.target].forEach((v) => { if (v != null && Number.isFinite(Number(v))) prices.push(Number(v)); });
  ...
  const firstMinutes = minutesFromMidnight(points[0].time), lastMinutes = minutesFromMidnight(points[points.length - 1].time), totalMinutes = Math.max(lastMinutes - firstMinutes, 1);
  const xForMinutes = (minutes) => padX + ((minutes - firstMinutes) / totalMinutes) * (width - padX * 2);
  const windowStarts = [[585,"09:45"],[630,"10:30"],[675,"11:15"],[720,"12:00"],[765,"12:45"],[810,"13:30"],[855,"14:15"],[900,"15:00"],[945,"15:45"]];
  const boundaryLines = windowStarts.filter(([m]) => m >= firstMinutes && m <= lastMinutes).map(([m,label]) => { const bx=xForMinutes(m); return `<path d="M${bx.toFixed(1)} 20V330" stroke="#6f86a1" stroke-width="1" stroke-dasharray="3 6"/><text x="${Math.min(bx+5,925).toFixed(1)}" y="318" fill="#91a4bd" font-size="10">${label}</text>`; }).join("");
  const anchorX = 585 >= firstMinutes && 585 <= lastMinutes ? xForMinutes(585) : null;
...
async function runPaperSimulation() {
  ...
  try {
    const risk=Number($("risk")?.value)||1, rr=Number($("rr")?.value)||2;
    ...
    const data=await response.json(); renderSummary(data.summary); renderTrades(data.trades);
    const stateResponse=await fetch(`${API}/api/execution-state?risk_reward=${encodeURIComponent(rr)}`); let state={};
    if(stateResponse.ok){state=await stateResponse.json();renderExecutionState(state);} renderChart(data.bars,state); setText("model","READY");
  } catch(error){setText("model","ERROR");const body=$("body");if(body)body.innerHTML=`<tr><td colspan="8" class="empty">Backend connection failed: ${error.message}</td></tr>`;}
  finally{if(run){run.disabled=false;run.textContent="Run paper simulation";}}
}

window.addEventListener("load",()=>{updateClock();setInterval(updateClock,1000);loadHistoricalData();const run=$("run");if(run)run.onclick=runPaperSimulation;});
